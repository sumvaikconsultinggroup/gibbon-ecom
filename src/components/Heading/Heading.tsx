...
const Heading: React.FC<HeadingProps> = ({
  ...
  return (
    <div className={clsx('relative flex flex-col  font-family-antonio justify-between sm:flex-row sm:items-end', className)}>
      <div className={clsx(isCenter && 'mx-auto flex w-full flex-col items-center text-center')}>
        <Level className={clsx(isCenter && 'justify-center', fontClass)} {...args}>
          ...
          {headingDim ? (
            <>
              <span>{'. '}</span>
              <span className="text-neutral-400 font-family-antonio">{headingDim}</span>
            </>
          ) : null}
        </Level>
        {!!description ? (
